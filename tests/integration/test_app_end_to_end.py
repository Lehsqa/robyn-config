...
def run_make_migration(project_dir: Path, design: str, orm: str) -> None:
    ...
    try:
        if orm == "tortoise":
            # For Tortoise, we cannot generate migrations locally on SQLite that are compatible with Postgres (e.g. SERIAL vs AUTOINCREMENT).
            # Instead, we rely on `aerich init-db` running inside the Docker container (via prod.py) to create the schema from models.
            # To do this, we must ensure NO migration files exist, so `init-db` generates a fresh schema including the new Product entity.
            migrations_dir = project_dir / "src" / "app" / "infrastructure" / "database" / "migrations" / "models"
            if design == "mvc":
                migrations_dir = project_dir / "src" / "app" / "models" / "migrations" / "models"
            
            if migrations_dir.exists():
                shutil.rmtree(migrations_dir)
            # We do NOT run `make makemigration` for Tortoise.
            return
        else:
            # SQLAlchemy
            subprocess.run(
            ...
            # Generate new migration (for Product)
            subprocess.run(
                ["make", "makemigration"],
                cwd=project_dir,
                check=True,
