...
class ModelAdmin:
    ...
    def __init__(self, model: Type[Model]):
        ...
        self.allow_import = getattr(
            self, "allow_import", False
        )  # 是否允许导入
        self.import_fields = getattr(
            self, "import_fields", []
        )  # 允许导入的字段
        # 初始化其他配置
        ...
        if search and self.search_fields:
            ...
            for field in self.search_fields:
                try:
                    ...
                    print(
                        f"Error building search query for {field.name}: {str(e)}"
                    )
                    continue
        ...
        for filter_field in filter_fields:
            ...
            if filter_value:
                try:
                    ...
                    print(
                        f"Error building filter query for {filter_field.name}: {str(e)}"
                    )
                    continue
        return queryset
            ...
            "inlines": [
                {
                    ...
                    "fields": [
                        field.to_dict() for field in inline.table_fields
                    ],
                    "title": getattr(
                        inline.model.Meta, "description", inline.verbose_name
                    ),
                }
    ...
    async def get_inline_data(self, parent_id: str, inline_model: str):
        ...
        try:
            ...
            inline = next(
                (
                    i
                    for i in self._inline_instances
