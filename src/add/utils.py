...
def _render_template_file(source: Path, target: Path, context: dict[str, str]) -> None:
...
def _add_table_to_tables_py(
    tables_path: Path, name: str, name_capitalized: str, orm: str, context: dict[str, str]
) -> None:
    ...
    template_file = ADD_MODULE_ROOT / "ddd" / "infrastructure" / orm / f"__name___table.py.jinja2"
...
def _add_ddd_templates(
    ...
    repo_template = templates_path / "infrastructure" / orm / "repository" / "__name__.py.jinja2"
    if repo_template.exists():
        repo_target = app_path / "infrastructure" / "database" / "repository" / f"{name}.py"
        ...
        repo_init = app_path / "infrastructure" / "database" / "repository" / "__init__.py"
        _update_init_file(
            repo_init,
...
def _append_class_to_file(
    file_path: Path, template_path: Path, context: dict[str, str], class_name: str
) -> None:
...
def _ensure_import_from(file_path: Path, module: str, import_item: str) -> None:
...
def _add_mvc_templates(
    ...
    table_template = templates_path / "models" / orm / "__name___table.py.jinja2"
    table_class = f"{name_capitalized}Table"
    if models_file.exists() and table_template.exists():
        _append_class_to_file(models_file, table_template, context, table_class)
    ...
    repo_template = templates_path / "models" / orm / "__name___repository.py.jinja2"
    ...
    if repo_file.exists() and repo_template.exists():
        ...
        _update_init_file(repo_file, import_line=f"from .models import {table_class}", export_name="") # abusing _update_init_file for import injection
        ...
        if f"from .models import" in repo_content and table_class not in repo_content:
...
def add_business_logic(project_path: Path, name: str) -> list[str]:
    ...
    name_capitalized = "".join(word.capitalize() for word in name_lower.split("_"))
    
    if design == "ddd":
        return _add_ddd_templates(project_path, name_lower, name_capitalized, orm)
    elif design == "mvc":
        return _add_mvc_templates(project_path, name_lower, name_capitalized, orm)
    else:
